...
Sub Conv_V50_SV_syx()
    ...
    Sheets("Menu").Select
    numBank = Cells(86, 5).Value
    strFilePath = Cells(87, 5).Value
    strFileName = Cells(88, 5).Value
...
Sub Conv_V50_MV_syx()
    ...
    Else
        ...
        If Dir(strFile) = "" Then
            ...
            Call Write_V50_syx2(32, strFile, numBank)
            Sheets("Menu").Select
            MsgBox "Sysexデータの書き出しが完了しました。"
...
Sub Write_V50_syx2(DataSet As Integer, filename As String, numBank As Integer)
    ...
    Dim OP1_EGSFT, OP1_FIXRM, OP1_FIX, OP1_FIXRG, OP1_OSW, OP1_FINE
    Dim OP2_EGSFT, OP2_FIXRM, OP2_FIX, OP2_FIXRG, OP2_OSW, OP2_FINE
    Dim OP3_EGSFT, OP3_FIXRM, OP3_FIX, OP3_FIXRG, OP3_OSW, OP3_FINE
    Dim OP4_EGSFT, OP4_FIXRM, OP4_FIX, OP4_FIXRG, OP4_OSW, OP4_FINE
    ...
    mv_hdr = "F0 43 10 24 07 " & "0" & numBank & " F7 " & "F0 43 00 04 20 00"
    chksum = 0
    data_str = ""
    Init_Voice = "1F 1F 00 0F 0F 00 00 00 04 03 1F 1F 00 0F 0F 00 00 00 04 03 1F 1F 00 0F 0F 00 00 00 04 03 1F 1F 00 0F 0F 00 00 5A 04 03 00 23 00 00 00 62 18 04 04 00 28 32 00 00 00 32 00 49 4E 49 54 20 56 4F 49 43 45 63 63 63 32 32 32 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 32 64 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"
    ...
    'SysexDataシートからデータを取得
        ...
        OP1_EGSFT = Cells(sr, sc + 85).Value
        OP1_FIXR = Cells(sr, sc + 86).Value
        OP1_FIXRG = Cells(sr, sc + 87).Value
        OP1_OSW = Cells(sr, sc + 88).Value
        OP1_FINE = Cells(sr, sc + 89).Value
        
        OP2_EGSFT = Cells(sr, sc + 90).Value
        OP2_FIXR = Cells(sr, sc + 91).Value
        OP2_FIXRG = Cells(sr, sc + 92).Value
        OP2_OSW = Cells(sr, sc + 93).Value
        OP2_FINE = Cells(sr, sc + 94).Value

        OP3_EGSFT = Cells(sr, sc + 95).Value
        OP3_FIXR = Cells(sr, sc + 96).Value
        OP3_FIXRG = Cells(sr, sc + 97).Value
        OP3_OSW = Cells(sr, sc + 98).Value
        OP3_FINE = Cells(sr, sc + 99).Value

        OP4_EGSFT = Cells(sr, sc + 100).Value
        OP4_FIXR = Cells(sr, sc + 101).Value
        OP4_FIXRG = Cells(sr, sc + 102).Value
        OP4_OSW = Cells(sr, sc + 103).Value
        OP4_FINE = Cells(sr, sc + 104).Value
        
        REV = Cells(sr, sc + 106).Value
        FC_Pitch = Cells(sr, sc + 107).Value
        FC_AMP = Cells(sr, sc + 108).Value
        AT_Pitch = Cells(sr, sc + 109).Value
        AT_AMP = Cells(sr, sc + 110).Value
        AT_PBias = Cells(sr, sc + 111).Value
        AT_EGBias = Cells(sr, sc + 112).Value
        
        E_PRST = Cells(sr, sc + 114).Value
        E_TIME = Cells(sr, sc + 115).Value
        E_BAL = Cells(sr, sc + 116).Value
        E_SEL = Cells(sr, sc + 117).Value
        BAL = Cells(sr, sc + 118).Value
        OUT_LV = Cells(sr, sc + 119).Value
        ST_MIX = Cells(sr, sc + 120).Value
        E_PRM1 = Cells(sr, sc + 121).Value
        E_PRM2 = Cells(sr, sc + 122).Value
        E_PRM3 = Cells(sr, sc + 123).Value
        
        OP1_FIXRM = Cells(sr, sc + 125).Value
        OP2_FIXRM = Cells(sr, sc + 126).Value
        OP3_FIXRM = Cells(sr, sc + 127).Value
        OP4_FIXRM = Cells(sr, sc + 128).Value
        ...
        'V2 V50用パラメーターを付加（バイト）
        
        If DataSet = 1 Then
        
            data_str = data_str & " " & HEX2(OP_ON)
            
            data_str = data_str & " " & HEX2(OP4_EGSFT)
            data_str = data_str & " " & HEX2(OP4_FIX)
            data_str = data_str & " " & HEX2(OP4_FIXRG)
            data_str = data_str & " " & HEX2(OP4_OSW)
            data_str = data_str & " " & HEX2(OP4_FINE)
            
            data_str = data_str & " " & HEX2(OP2_EGSFT)
            data_str = data_str & " " & HEX2(OP2_FIX)
            data_str = data_str & " " & HEX2(OP2_FIXRG)
            data_str = data_str & " " & HEX2(OP2_OSW)
            data_str = data_str & " " & HEX2(OP2_FINE)

            data_str = data_str & " " & HEX2(OP3_EGSFT)
            data_str = data_str & " " & HEX2(OP3_FIX)
            data_str = data_str & " " & HEX2(OP3_FIXRG)
            data_str = data_str & " " & HEX2(OP3_OSW)
            data_str = data_str & " " & HEX2(OP3_FINE)

            data_str = data_str & " " & HEX2(OP1_EGSFT)
            data_str = data_str & " " & HEX2(OP1_FIX)
            data_str = data_str & " " & HEX2(OP1_FIXRG)
            data_str = data_str & " " & HEX2(OP1_OSW)
            data_str = data_str & " " & HEX2(OP1_FINE)

            data_str = data_str & " " & HEX2(REV)
            data_str = data_str & " " & HEX2(FC_Pitch)
            data_str = data_str & " " & HEX2(FC_AMP)
            data_str = data_str & " " & HEX2(AT_Pitch)
            data_str = data_str & " " & HEX2(AT_AMP)
            data_str = data_str & " " & HEX2(AT_PBias)
            data_str = data_str & " " & HEX2(AT_EGBias)
            
            data_str = data_str & " " & HEX2(OP4_FIXRM)
            data_str = data_str & " " & HEX2(OP2_FIXRM)
            data_str = data_str & " " & HEX2(OP3_FIXRM)
            data_str = data_str & " " & HEX2(OP1_FIXRM)
            
            data_str = data_str & " " & HEX2(E_PRST)
            data_str = data_str & " " & HEX2(E_TIME)
            data_str = data_str & " " & HEX2(E_BAL)
            data_str = data_str & " " & HEX2(E_SEL)
            data_str = data_str & " " & HEX2(BAL)
            data_str = data_str & " " & HEX2(OUT_LV)
            data_str = data_str & " " & HEX2(ST_MIX)
            data_str = data_str & " " & HEX2(E_PRM1)
            data_str = data_str & " " & HEX2(E_PRM2)
            data_str = data_str & " " & HEX2(E_PRM3)
            
            data_str = data_str & " " & HEX2(LS_SGN)
            data_str = data_str & " " & "00"
        
        ElseIf DataSet = 32 Then
        
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP4_FIXRM, 1) & DecToBin(OP4_EGSFT, 2) & DecToBin(OP4_FIX, 1) & DecToBin(OP4_FIXRG, 3)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP4_OSW, 3) & DecToBin(OP4_FINE, 4)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP2_FIXRM, 1) & DecToBin(OP2_EGSFT, 2) & DecToBin(OP2_FIX, 1) & DecToBin(OP2_FIXRG, 3)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP2_OSW, 3) & DecToBin(OP2_FINE, 4)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP3_FIXRM, 1) & DecToBin(OP3_EGSFT, 2) & DecToBin(OP3_FIX, 1) & DecToBin(OP3_FIXRG, 3)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP3_OSW, 3) & DecToBin(OP3_FINE, 4)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP1_FIXRM, 1) & DecToBin(OP1_EGSFT, 2) & DecToBin(OP1_FIX, 1) & DecToBin(OP1_FIXRG, 3)))
            data_str = data_str & " " & HEX2(BinToDec(DecToBin(OP1_OSW, 3) & DecToBin(OP1_FINE, 4)))
            ...
            data_str = data_str & " " & "00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"
    ...
    If DataSet = 1 Then
        ReDim chksum_byte(UBound(chksum_data) - 6)
    ...
    ElseIf DataSet = 32 Then
        ReDim chksum_byte(UBound(chksum_data) - 13)
        
        For k = 13 To UBound(chksum_data)
            chksum_byte(k - 13) = "&h" & chksum_data(k)
        Next k
